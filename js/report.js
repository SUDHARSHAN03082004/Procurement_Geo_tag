...
function resetForm() {
    ...
    if (punchStatus) punchStatus.textContent = '';
...
// ── Merged: Share + Submit + Optional Punch ───────────────────
async function submitReportAndPunch() {
    if (!STATE.capturedPhoto) {
        showMessage('📷 Please capture a photo first', 'error');
        return;
    }
    ...
    // 1. Submit the report to Google Sheets
    submitRecord();

    // 2. Share to Telegram (photo + data)
    await shareReport();

    // 3. If punch action was selected, record it to attendance sheet
    if (STATE.punchAction) {
        const shift = STATE.punchAction;
        const punchData = {
            action: 'punchIn',
            supervisorId: STATE.currentUser?.id || 'unknown',
            supervisorName: STATE.currentUser?.name || 'unknown',
            ...
            lat: STATE.currentPosition?.latitude || null,
            lon: STATE.currentPosition?.longitude || null,
            village: STATE.currentLocationName || '',
            date: formatDate(now),
        };
        apiPost('punchIn', punchData).then(res => {
            if (res.ok) console.log(`${shift} punch recorded`);
        });
    }
